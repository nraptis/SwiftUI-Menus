//
//  MagicalSliderViewModel+ContinuousStartRotation.swift
//  Jiggle3
//
...
@Observable class MagicalSliderViewModelContinuousStartRotation: MagicalSliderViewModel {
        ...
        if let jiggleViewModel = ApplicationController.shared.jiggleViewModel {
            if let selectedJiggle = jiggleViewModel.getSelectedJiggle() {
                ...
                if jiggleViewModel.getContinuousDraggingStatus() {
                    refreshDisabled(value: selectedJiggle.continuousStartRotation)
                    return
                }
                
                if jiggleViewModel.isSliderActiveBesides(thisSlider: .sliderContinuousStartRotation) {
                    refreshDisabled(value: selectedJiggle.continuousStartRotation)
                } else {
                    refreshEnabled(value: selectedJiggle.continuousStartRotation)
                }
            } else {
                refreshDisabled()
            }
    ...
    deinit {
        if ApplicationController.DEBUG_DEALLOCS {
            print("MagicalSlider ContinuousStartRotation - Dealloc")
        }
    }
    ...
    override func handleSlideStarted(percent: CGFloat) {
        ...
        if let jiggleViewModel = ApplicationController.shared.jiggleViewModel {
            ...
            jiggleViewModel.notifySliderStartedContinuousStartRotation(value: value)
        }
    }
        ...
        if let jiggleViewModel = ApplicationController.shared.jiggleViewModel {
            ...
            if jiggleViewModel.isAnimationContinuousAppliedToAll {
                ...
                for jiggleIndex in 0..<jiggleDocument.jiggleCount {
                    ...
                    jiggle.continuousStartRotation = value
                }
            } else {
                if let selectedJiggle = jiggleViewModel.getSelectedJiggle() {
                    selectedJiggle.continuousStartRotation = value
                }
            }
        }
    ...
    override func handleSlideFinished(percent: CGFloat) {
        ...
        if let jiggleViewModel = ApplicationController.shared.jiggleViewModel {
            ...
            jiggleViewModel.notifySliderFinishedContinuousStartRotation(value: value)
        }
    }
